...
* Plotly charts: nbsphinx_prolog now sets `require.config` paths for plotly.js 2.8.3, and OneTrust AutoBlock is loaded at the end of the body so it does not block the Plotly CDN. Stored plotly.py 6 cell outputs that request `plotly-3.1.0.min` (invalid without `.js`; plotly.js 3.x anyway) are rewritten at build time to `https://cdn.plot.ly/plotly-2.8.3.min.js`.
* vis.js sidecar HTML copied by `make extra-html` now gets `<!DOCTYPE html>` when missing, so network iframes are not rendered in Quirks Mode.
